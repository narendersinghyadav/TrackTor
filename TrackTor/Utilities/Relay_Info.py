...
import calendar
from TrackTor.Utilities.dna import get_info_relay,status,build_flags,path,created,is_alive
        ...
        if (Name != 'Circuit'):
            for Count in range (Row_Count, Row_Count + len(Type)):
                ...
                if (Name == 'Outbound' or Name == 'Exit'):
                    ...
                    dest_address = get_info_relay('address')
                    src_address = Type[Count-Row_Count][0]
                ...
                Src_Country2 = str(get_info_relay('ip-to-country/%s' %src_address ))
                Dest_Country2 = str(get_info_relay('ip-to-country/%s' %dest_address))
            ...
            count = Count
            for Count in range (Row_Count, Row_Count+len(Type)):
                if (status(Type[count-Row_Count]) == 'BUILT'):
                    if (('ONEHOP_TUNNEL' not in build_flags(Type[count-Row_Count]))):
                        #print ("Something")
                        GuardNode_fingerprint = str(path(Type[count-Row_Count])[0][0])[1:].upper()
                        MiddleNode_fingerprint = str(path(Type[count-Row_Count])[1][0])[1:].upper()
                        EndNode_fingerprint = str(path(Type[count-Row_Count])[2][0])[1:].upper()
                        Category = Name
                    else:
                        EndNode_fingerprint = str(path(Type[count-Row_Count])[0][0])[1:].upper()
                        Category = 'OHT_Circuit'

                    Src_IP =get_info_relay('address')
                    if (Src_IP == None):
                        Src_IP = '127.0.0.1'
                    ...
                    if address:
                        ...
                        locale =get_info_relay('ip-to-country/%s' %address )
                        if locale != '??':
                    ...
                    Src_Country2 = str(get_info_relay('ip-to-country/%s' %Src_IP))
                    if (Src_Country2.isalpha() == False or Src_Country2 == 'None'):
                        Src_Country = "None"
                    else:
                        Src_Country1 = pycountry.countries.get(alpha_2 = str.upper(Src_Country2))
                    ...
                    TS = str(created(Type[count-Row_Count]))
